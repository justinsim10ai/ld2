...
export const FACTORS = {
  ...
  barrelRate: {
    ...
    description: 'Share of batted balls hit in the ideal exit-velo + launch-angle window — the best HR predictor.',
    longExplanation: [
      'A "barrel" is Statcast’s name for a batted ball hit with the combination of exit velocity and launch angle that historically produces the most damage — at least a .500 average and 1.500 slugging. Barrel rate is the share of a hitter’s batted balls that qualify.',
      'It’s the single most predictive batted-ball metric for home runs: barrels are essentially the contact that leaves the yard. League average is around 8%; elite power hitters run 12–16%+.',
      'We z-score against the 8% baseline and weight it heavily in the Home Run model. Pulled from Baseball Savant; higher is better.',
    ],
  },
  hardHitPct: {
    id: 'hardHitPct',
    ...
    keyColumn: true,
    format: (v) => `${v.toFixed(1)}%`,
    description: 'Share of batted balls struck at 95+ mph exit velocity — raw contact authority.',
    longExplanation: [
      'Hard-hit rate is the percentage of a hitter’s batted balls with an exit velocity of at least 95 mph. Hard contact turns into extra bases far more often than soft contact, which is why it anchors the Total Bases model alongside xSLG.',
      'League average is roughly 38–39%; the game’s best run north of 50%. It’s a stickier, lower-variance signal than slugging itself.',
      'From Baseball Savant; we z-score against ~38.5%. Higher is better.',
    ],
  },
  whiffPct: {
    id: 'whiffPct',
    ...
    keyColumn: true,
    format: (v) => `${v.toFixed(1)}%`,
    description: 'How often hitters swing and miss against this pitcher — the leading indicator of strikeouts.',
    longExplanation: [
      'Whiff rate is the share of swings against a pitcher that come up empty. It’s the "stuff" signal: pitchers who miss bats strike hitters out, and whiff% stabilizes faster and predicts future strikeouts better than the outcome-based K/9.',
      'We don’t add it as a separate weighted term — the strikeout projection is multiplicative — but use it as a "stuff" multiplier that nudges expected strikeouts up or down based on how far a pitcher’s whiff% sits from the ~24.5% league average (capped to a sane band).',
      'From Baseball Savant; higher is better for the pitcher.',
    ],
  },
  recentHr: {
    id: 'recentHr',
