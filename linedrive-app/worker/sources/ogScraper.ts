...
async function getSitemapUrls(env: Env): Promise<string[]> {
  try {
    const cached = await env.LINEDRIVE_KV.get<SitemapCache>(SITEMAP_KV_KEY, "json");
...
async function scrapeGame(env: Env, url: string, gamePk: number): Promise<ScrapedGameMarkets | null> {
  ...
  try {
    const cached = await env.LINEDRIVE_KV.get<GameCache>(cacheKey, "json");
  ...
  try {
    ...
    await env.LINEDRIVE_KV.put(cacheKey, JSON.stringify(cache), {
