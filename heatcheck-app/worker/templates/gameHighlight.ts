...
const SIZE = 1080;
const OVER_COLOR = "#7AE3B8";
const UNDER_COLOR = "#F6A05E";

export function gameHighlightNode(props: GameHighlightProps) {
  const accent = CATEGORY_ACCENT.game;
  const p = props.pick;
  const modelRuns = p.score;
  const marketLine = p.marketOverLine;
  const delta = marketLine !== null && marketLine !== undefined ? modelRuns - marketLine : null;
  const side = delta === null ? null : delta >= 0 ? "OVER" : "UNDER";
  const sideColor = side === "OVER" ? OVER_COLOR : side === "UNDER" ? UNDER_COLOR : COLORS.muted;
  const deltaLabel = delta === null ? "" : `${delta >= 0 ? "+" : "−"}${Math.abs(delta).toFixed(1)} R`;
  ...
  return h(
    ...
    h(
      ...
      h(
        "div",
        { style: { display: "flex", fontSize: 30, color: COLORS.muted, marginTop: 12 } },
        p.matchup,
      ),
    ),
    // Model vs Market row
    h(
      ...
      h(
        ...
        h("div", { style: { display: "flex", fontSize: 22, color: COLORS.muted, letterSpacing: 2 } }, "MODEL"),
        h(
          "div",
          {
            style: {
          ...
          `${modelRuns.toFixed(1)} R`,
        ),
      ),
      marketLine !== null && marketLine !== undefined
        ? h(
            "div",
    ...
    // Big delta indicator (OVER / UNDER + amount)
    side
      ? h(
          "div",
          {
            style: {
              backgroundColor: `${sideColor}14`,
              border: `2px solid ${sideColor}66`,
              borderRadius: 22,
              padding: "20px 32px",
              display: "flex",
              alignItems: "center",
      ...
      : h("div", { style: { display: "flex", marginBottom: 24 } }, ""),
    // Signals
    h(
      "div",
      ...
      ...p.signals.slice(0, 3).map((s) =>
        h(
          "div",
          { style: { display: "flex", alignItems: "center", fontSize: 28, marginBottom: 10 } },
          h(
            "div",
